...
Wrapper for D2XX library function `FT_ResetDevice`.
...
julia> FT_ResetDevice(handle) # set reset command to device...

julia> FT_Close(handle)
```
"""
function FT_ResetDevice(ftHandle::FT_HANDLE)
  status = ccall(cfunc[:FT_ResetDevice], cdecl, FT_STATUS, (FT_HANDLE,),
                                                           ftHandle)
  FT_STATUS_ENUM(status) == FT_OK || throw(FT_STATUS_ENUM(status))
  return
end
